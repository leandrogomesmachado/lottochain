    ...
    state: {
        ...
        games: [{
            ...
            },
            {
                ...
                p2: '<p>50% for the daily prize address;<br>20% for the weekly prize address;<br>10% for the monthly prize address;<br>10% for the monthly super ticket holder prize address;<br>10% for Lottochain address (marketing, infrastructure, maintenance,legal support, fees)<br></p><p>The user addresses are stored and indexed in the Smart Contract, copied across different structures for each prize drawing periodicity (daily, weekly and monthly). Daily Prizes are drawn every day around 2 AM GMT. Weekly prizes, every Saturday around 2 AM GMT. Monthly and Super Ticket prizes, every 28th at 2 AM GMT.</p><p>The core of Lottochain (ticketing, prize drawing and funds transferring) resides in Htmlcoin Blockchain Smart Contracts, in what is called a Decentralized Application (DApp), developed with Solidity language. Each Smart Contract is an autonomous unity of processing running on the blockchain structure, hence immutable and auditable by anyone. Our Smart Contracts are being developed under all best-practice standards recommended by the platform for algorithm security and optimization, what can be verified since all codes are open-source and will be available on GitHub. They are responsible for ticket intake, drawing and funds transferences.</p><p>In order to have a fair drawing system, we need a source of randomness which cannot be achieved through Htmlcoin Smart Contracts, as they need to be deterministic, thus not being able to generate random numbers. The solution found was to refer to the Bitcoin and Htmlcoin Blockchains, from where we will take the hash of the first block mined after a pre-determined time in the future. In possession of that hash, we can then derive a number from it, which will be the index of the winning ticket. The prize transfer will happen immediately after the draw to the winning wallet.</p>',
                p3: '<p><h3>Draws in details</h3></p><p>Firstly, let us clear the concept of <i><b>confirmation</b></i> in blockchains. A transaction has one confirmation when it is registered in a block on the blockchain. When the next block gets mined, that transaction has then two confirmations, and so on.</p><p>In the concept of blockchain consensus, blocks can be discarded while they still have a low number of confirmations, and for a block to be considered "irreversible" each entity defines their own critera of how many confirmations are needed for a transaction to be considered as definitive part of the blockchain.</p><p>Each block has its own <b>hash</b>, which is a unique long sequence of characters that plays an important role on the blockchain consistency.</p><p>Block hashes cannot be predicted and are known only be the time the block is mined. That is the source we need to seed our draw.</p><p>On the HTMLCoin Blockchain (Althash), we consider that a block is "safely confirmed" when it reaches <b>10 confirmations</b>.<br>On the Bitcoin Blockchain, we consider <b>6 confirmations</b>.</p><p>Lottochain takes the following steps for each draw:</p><p><ol><li><b>1 AM GMT:</b> All ticket purchasing is paused.</li><br><li><b>Next step:</b><ol type="a"><li><b>Daily draw:</b> We pick the <b>first</b> confirmed block from both HTMLCoin and Bitcoin networks, mined after 1 AM and with the minimum confirmations agreed above.</li><li><b>Weekly draw (every Saturday):</b> We pick the <b>second</b> confirmed block from both HTMLCoin and Bitcoin networks, mined after 1 AM and with the minimum confirmations agreed above.</li><li><b>Monthly draw (every 28th):</b> We pick the<b>third</b>confirmed block from both HTMLCoin and Bitcoin networks, mined after 1 AM and with the minimum confirmations agreed above.</li><li><b>Super Ticket draw (every 28th):</b> We pick the <b>fourth</b> confirmed block from both HTMLCoin and Bitcoin networks, mined after 1 AM and with the minimum confirmations agreed above.</li></ol></li><br><li>With both block <b>hashes</b> on hands, we trigger the Lottochain Smart Contract draw function, which will:<ol type="a"><li>Convert each hash into a long integer.</li><li>Sum both integers into an even longer integer.</li><li>Take the rest of the division of that big integer by the number of participants in the draw.</li><li>The result is used to point what is the position of our winner in the list of participants we keep in the smart contract.</li><li>We pick the winning wallet address and transfer the full prize into it.</li><li>We clear the list of participants, so it is ready to start being filled again with participants of the next draw.</li></ol></li><br><li>All ticket purchasing is resumed.</li></ol></p>',
            },
            {
                id: 3,
                name: 'Token Tower',
