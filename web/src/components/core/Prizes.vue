          ...
          >
            ...
            <template v-else-if="lotteryActive">
              <v-list-item>
                ...
                <v-list-item-content>
                  ...
                  <v-list-item-subtitle>
                    {{ (Math.round(dailyPrize * 100) / 100).toLocaleString("en-US", {style: "decimal", minimumFractionDigits: 2}) }} HTML ({{ dailyPrizeUSD }} USD)
                  </v-list-item-subtitle>
                </v-list-item-content>
              </v-list-item>
              <v-list-item>
                ...
                <v-list-item-content>
                  ...
                  <v-list-item-subtitle>
                    {{ (Math.round(weeklyPrize * 100) / 100).toLocaleString("en-US", {style: "decimal", minimumFractionDigits: 2}) }} HTML ({{ weeklyPrizeUSD }} USD)
                  </v-list-item-subtitle>
                </v-list-item-content>
              </v-list-item>
              <v-list-item>
                ...
                <v-list-item-content>
                  ...
                  <v-list-item-subtitle>
                    {{ (Math.round(monthlyPrize * 100) / 100).toLocaleString("en-US", {style: "decimal", minimumFractionDigits: 2}) }} HTML ({{ monthlyPrizeUSD }} USD)
                  </v-list-item-subtitle>
                </v-list-item-content>
              </v-list-item>
              <v-list-item>
                ...
                <v-list-item-content>
                  ...
                  <v-list-item-subtitle>
                    {{ (Math.round(superTicketsPrize * 100) / 100).toLocaleString("en-US", {style: "decimal", minimumFractionDigits: 2}) }} HTML ({{ superTicketsPrizeUSD }} USD)
                  </v-list-item-subtitle>
                </v-list-item-content>
              </v-list-item>
              ...
              <v-list-item>
                ...
                <v-list-item-content>
                  <v-list-item-title>
                    <span class="font-weight-bold">Prizes so far</span>
                  </v-list-item-title>
    ...
    methods: {
      ...
      counterMonthly () {
        ...
        if (baseDate.getDate() > 28 || (baseDate.getDate() === 28 && baseDate.getHours() >= 1)) {
          baseDate.setMonth(baseDate.getMonth() + 1, 28)
        } else {
          baseDate.setDate(28)
        }
        baseDate.setHours(1)
        ...
        try {
          ...
          if (self.lotteryActive) {
            ...
            decodedResult = await this.callContractFunction(
              contractAddress,
              abiJson,
              'accumulatedDailyPrize',
              [],
            )
            const accumulatedDailyPrize = parseFloat(decodedResult[0]) / 100000000

            decodedResult = await this.callContractFunction(
              contractAddress,
              abiJson,
              'accumulatedWeeklyPrize',
              [],
            )
            const accumulatedWeeklyPrize = parseFloat(decodedResult[0]) / 100000000

            decodedResult = await this.callContractFunction(
              contractAddress,
              abiJson,
              'accumulatedMonthlyPrize',
              [],
            )
            const accumulatedMonthlyPrize = parseFloat(decodedResult[0]) / 100000000

            decodedResult = await this.callContractFunction(
              contractAddress,
              abiJson,
              'accumulatedSuperTicketsPrize',
              [],
            )
            const accumulatedSuperTicketsPrize = parseFloat(decodedResult[0]) / 100000000

            self.totalPrizes = (accumulatedDailyPrize + accumulatedWeeklyPrize + accumulatedMonthlyPrize + accumulatedSuperTicketsPrize)
            self.totalPrizesUSD = (self.totalPrizes / (self.ticketPrice / 5)).toLocaleString('en-US', { style: 'currency', currency: 'USD' })
            self.totalPrizes = self.totalPrizes.toLocaleString('en-US', { style: 'decimal', minimumFractionDigits: 2 })
          }
