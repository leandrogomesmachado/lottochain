  ...
  <section>
    ...
    <div class="text-center pa-3">
      ...
      <div
        ...
        class="pa-5 headline"
      >
        <b>$20 USD = {{ parseFloat(superTicketPrice).toLocaleString("en-US", {style: "decimal", minimumFractionDigits: 2}) }} HTML</b>
      </div>
      <div>
        <p>
          Lottochain Super Tickets (LOTTO Tokens) are indexed by USD and paid with
          <a
      ...
      >
        ...
        <template v-else>
          ...
          <v-row>
            <v-col>
              <v-spacer />
              <span class="subtitle-1">Your valid Super Tickets:</span>
              <v-spacer />
            </v-col>
          </v-row>
          <v-row>
    ...
    >
      <v-card
        ...
        color="blue-grey darken-3"
      >
        The USD/HTML rate is set based on the average price and market volume, and it may be sometimes a little bit different from your prefered exchange. After fast dump and pump on rates, there might be a delay for it to get updated on our system. The Super Ticket price will always respect the rate shown here, and not the market price.
      </v-card>
    </v-dialog>
    <v-dialog
    ...
    >
      <v-card
        ...
        color="blue-grey darken-3"
      >
        2.01 HTML were added to the value for the network fees (transaction gas + mining fee). A part of this amount will be reimbursed after the transaction is completed.
      </v-card>
    </v-dialog>
    <v-dialog
      v-model="disabledSaleDialog"
      width="600px"
    >
      <v-card
        class="pa-5"
      ...
      >
        <center>
          <p class="title">
            Lottochain Super Tickets purchase has been paused.
          </p>
          <p>
            This status happens mostly during maintenance periods.
            <br>
    ...
    >
      ...
      >
        <center>
          <p class="title">
            The Super Ticket price has been updated.
          </p>
          <p>
            Please select your Super Ticket quantity again and check for new values.
          </p>
        </center>
      </v-card>
    </v-dialog>
    ...
    >
      ...
      >
        ...
        >
          <center>
            Do you confirm the purchase of <br>{{ amount }} Super Ticket for {{ finalPrice }} HTML ?
          </center>
        </span>
        <span
          v-else
        ...
        >
          <center>
            Do you confirm the purchase of <br>{{ amount }} Super Tickets for {{ finalPrice }} HTML ?
          </center>
        </span>
        <v-card-actions>
          <v-spacer />
          ...
          <v-row>
            <v-col>
              <center>
                <span class="pa-3 white--text">
                  <p>
                    Your Super Tickets have been purchased.
                    <br>
                    You are now participating on <b>every Super Ticket draw</b>, monthly, until you win a prize.
                  </p>
                  <p>
                    <b>Good luck!</b>
                  </p>
...
<script>
  ...
  const contractAddress = config.getNetwork() === 'mainnet' ? '9675f371f1e36ca4d26ea8acd9216067732f7292' : '9ea8b6494f9718ae0e934649727895c1b4ad8bea'

  const abiJson = JSON.parse(
    '[{"constant": true, "inputs": [], "name": "saleActive", "outputs": [{"name": "", "type": "bool"} ], "payable": false, "stateMutability": "view", "type": "function"}, {"constant": true, "inputs": [], "name": "superTicketPrice", "outputs": [{"name": "", "type": "uint256"} ], "payable": false, "stateMutability": "view", "type": "function"}, {"constant": false, "inputs": [], "name": "buySuperTicket", "outputs": [], "payable": true, "stateMutability": "payable", "type": "function"} ]',
  )

  const lottochainContractAddress = config.getNetwork() === 'mainnet' ? 'ced6ccc8eead0970c8f178062779d5c547a53de7' : 'ac84457145f4e0330629a3f58032305fbfaa2854'

  const lottochainAbiJson = JSON.parse(
    '[{"constant": true, "inputs": [], "name": "superTicketPrice", "outputs": [{"name": "", "type": "uint256"} ], "payable": false, "stateMutability": "view", "type": "function"} ]',
  )

  const superTicketContractAddress = config.getNetwork() === 'mainnet' ? '030bcee2c0fac4423baa3d91190a892a882c0dff' : '82562c685a852289303ee2c89fe2fa4099bb13c2'
  ...
  export default {
    data () {
      return {
        ...
        disabledSaleDialog: false,
        loadingSaleStatus: true,
        saleActive: true,
        pkDialog: false,
    ...
    methods: {
      ...
      async restore () {
        ...
        try {
          ...
          await this.getCurrentSuperTicketsBalance()
        } catch (e) {
      ...
      async getCurrentSuperTicketsBalance () {
        var self = this
        try {
          const hrc20 = await server.currentNode().getHrc20(self.wallet.info.address)
          for (var i = 0; i < hrc20.length; i++) {
            if (hrc20[i].contract.contract_address === superTicketContractAddress) {
              self.currentBalanceSuperTickets = parseInt(hrc20[i].amount)
              break
            }
          }
        } catch (e) {
      ...
      calculateFinalPrice () {
        var self = this
        self.transactionPrice = parseFloat(self.amount) * parseFloat(self.superTicketPrice)
        if (parseFloat(self.transactionPrice + 2.01) > parseFloat(self.wallet.info.balance)) {
          alert('The final price is greater than your wallet balance!')
          self.amount = ''
          self.finalPrice = 0
        } else {
          self.finalPrice = (self.transactionPrice + 2.01).toLocaleString('en-US', { style: 'decimal', minimumFractionDigits: 2 })
        }
      },
      async refreshWallet () {
        var self = this
      ...
      async buySuperTickets () {
        var self = this
        const checkPrice = self.superTicketPrice
        self.confirmBuy = false
        await this.getInfo()
        if (checkPrice !== self.superTicketPrice) {
          self.finalPrice = 0
          self.amount = 0
          ...
          try {
            ...
            alert('Error sending transaction to the blockchain!')
      ...
      async getInfo () {
        var self = this
        self.loadingSuperTicketPrice = true
        try {
          var decodedResult = await this.callContractFunction(
            lottochainContractAddress,
            lottochainAbiJson,
            'superTicketPrice',
            [],
          )
          self.superTicketPrice = parseInt(decodedResult[0]) / 100000000
          self.loadingSuperTicketPrice = false
          ...
          self.saleActive = decodedResult[0]
          if (!self.saleActive) {
            self.disabledSaleDialog = true
            self.wallet = false
          }
          self.loadingSaleStatus = false
        } catch (e) {
          console.log('Error reading super ticket price: ' + e.stack || e.toString() || e)
          alert(e.message || e)
        }
      },
      closeTxConfirmation () {
        ...
        this.getInfo()
        this.refreshWallet()
        this.getCurrentSuperTicketsBalance()
      },
